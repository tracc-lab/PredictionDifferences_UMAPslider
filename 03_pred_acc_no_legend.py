...
def create_accuracy_plots_no_legend(config):
    ...
    for method in results_df['Feature_Selection_Method'].unique():
        ...
        # Apply color based on highlight status and accuracy
        merged_df['color'] = np.where(
            merged_df[config['highlight']['id_column']].isin(config['highlight']['ids']),
            merged_df['accuracy'],
            "background"
        )
        
        # Create plot with fixed ranges and NO LEGEND
        fig = px.scatter(
            merged_df,
            x='UMAP1',
            y='UMAP2',
        ...
        print(f"Saved {method} accuracy plot (no legend): {img_path}")
        print(f"Saved {method} accuracy data: {data_path}")
    
    print(f"\nAll accuracy plots (no legend) saved to: {output_dir}")

if __name__ == "__main__":
    config = load_config()
    create_accuracy_plots_no_legend(config)
